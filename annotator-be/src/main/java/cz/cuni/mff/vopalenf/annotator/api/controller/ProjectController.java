...
public class ProjectController {
    ...
    @Operation(summary = "Get all existing projects", description = "Retrieves a list of all projects available in the system.", responses = {
            @ApiResponse(responseCode = "200", description = "List of projects retrieved successfully", content = @Content(schema = @Schema(implementation = Project.class))),})
    ...
    @Operation(summary = "Get project by ID", description = "Retrieves a project based on the given ID.", parameters = {
            @Parameter(in = ParameterIn.PATH, name = "id", description = "ID of the project", required = true)}, responses = {
    ...
    @Operation(summary = "Delete project by ID", description = "Deletes a project based on the given ID.", parameters = {
            @Parameter(in = ParameterIn.PATH, name = "id", description = "ID of the project", required = true)}, responses = {
    ...
    @Operation(summary = "Update project by ID", description = "Updates a project based on the given ID and request body.", parameters = {
            @Parameter(in = ParameterIn.PATH, name = "projectId", description = "ID of the project", required = true)}, requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Project request object containing necessary details", required = true, content = @Content(schema = @Schema(implementation = ProjectRequest.class))), responses = {
    ...
    @Operation(summary = "Get all priorities", description = "Retrieves a list of all available priorities.", responses = {
    ...
    @Operation(summary = "Get all labels", description = "Retrieves a list of all labels.", responses = {
    ...
    @Operation(summary = "Add a new label", description = "Creates a new label with the given details.", requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Label request object containing necessary details", required = true, content = @Content(schema = @Schema(implementation = LabelRequest.class))), responses = {
    ...
    @Operation(summary = "Upload project data", description = "Creates a new project, saves a file to the filesystem, and creates a new database record.", requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(required = true, content = @Content(mediaType = "multipart/form-data")), responses = {
    ...
    public ResponseEntity<Project> manageFileUpload(
            @Parameter(description = "Name of the new project", required = true) @RequestPart("projectName") String projectName,
            @Parameter(description = "Deadline for project completion (yyyy-MM-dd format)", required = true) @RequestPart("deadline") String deadline,
            @Parameter(description = "Project priority (HIGH, MEDIUM, LOW)", required = true) @RequestPart("priority") String priority,
            @Parameter(description = "Team ID associated with the project", required = false) @RequestPart(value = "teamId", required = false) String teamId,
            @Parameter(description = "Compressed ZIP file containing log files and images", required = true) @RequestPart("file") MultipartFile file) {
        ProjectRequest projectRequest = ProjectRequest.builder().projectName(projectName)
                .deadline(LocalDate.parse(deadline)).priority(priority)
                .teamId(teamId != null ? Long.valueOf(teamId) : null).file(file).build();
        Project newProject = projectService.manageFileUpload(projectRequest);
        return ResponseEntity.ok(newProject);
    }

    @Operation(summary = "Annotate a single frame", description = "Annotates a specific frame in a project with a given label.", parameters = {
            ...
            @Parameter(name = "labelId", description = "Label ID to use for annotation", required = true, in = ParameterIn.PATH)}, responses = {
    ...
    @Operation(summary = "Annotate a range of frames", description = "Annotates multiple frames in a project with a given label.", parameters = {
            @Parameter(name = "projectId", description = "Project ID", required = true, in = ParameterIn.PATH),
    ...
    public ResponseEntity<Void> annotateProjectFramesInRange(@PathVariable Long projectId,
            @PathVariable Long startFrameId, @PathVariable Long endFrameId, @PathVariable Long labelId) {
        projectService.annotateProjectFramesInRange(projectId, startFrameId, endFrameId, labelId);
        return ResponseEntity.ok().build();
    }

    @Operation(summary = "Erase annotations in a range", description = "Removes annotations from a range of frames in a project.", parameters = {
            @Parameter(name = "projectId", description = "Project ID", required = true, in = ParameterIn.PATH),
            @Parameter(name = "startFrameId", description = "Starting frame ID", required = true, in = ParameterIn.PATH),
            @Parameter(name = "endFrameId", description = "Ending frame ID", required = true, in = ParameterIn.PATH)}, responses = {
    ...
    public ResponseEntity<Void> eraseAnnotationsInRange(@PathVariable Long projectId, @PathVariable Long startFrameId,
            @PathVariable Long endFrameId) {
        projectService.eraseAnnotationsInRange(projectId, startFrameId, endFrameId);
        return ResponseEntity.ok().build();
    }

    @Operation(summary = "Get all annotations for a project", description = "Retrieves a list of all annotations associated with a project.", parameters = {
            @Parameter(name = "projectId", description = "Project ID", required = true, in = ParameterIn.PATH)}, responses = {
                    ...
                    @ApiResponse(responseCode = "404", description = "Project not found"),})
    @PreAuthorize("hasAnyAuthority('ROLE_ADMIN', 'ROLE_USER')")
    @GetMapping(value = "/projects/{projectId}/annotations", produces = MediaType.APPLICATION_JSON_VALUE)
    ...
    @Operation(summary = "Get all project progresses", description = "Retrieves a list of all progress statuses for projects.", responses = {
    ...
    @Operation(summary = "Predict gestures for a project", description = "Sends raw sensor data to the AI service for gesture prediction and returns the predicted segments.", responses = {
            @ApiResponse(responseCode = "200", description = "AI prediction completed successfully", content = @Content(schema = @Schema(implementation = PredictionResponse.class))),})
    @PreAuthorize("hasAnyAuthority('ROLE_ADMIN', 'ROLE_USER')")
    @PostMapping("/projects/{projectId}/predict")
    public ResponseEntity<PredictionResponse> predictAI(@PathVariable Long projectId) {
        return ResponseEntity.ok(projectService.predictAI(projectId));
    }

...
public class UserService {
     ...
     *            UserRepository instance to use for database operations
     * @param userMapper
     *            UserMapper instance to map UserEntity to User and vice versa
     * @param teamMapper
     *            TeamMapper instance to map TeamEntity to Team and vice versa
     * @param passwordEncoder
     *            PasswordEncoder instance to encode and verify passwords
     */
    @Autowired
    public UserService(UserRepository userRepository, UserMapper userMapper, TeamMapper teamMapper,
    ...
    public List<User> getAllUsers() {
        return userRepository.findAll().stream()
                .map(userEntity -> userMapper.mapUser(userEntity, teamMapper.mapTeam(userEntity.getTeam()))).toList();
    }
    ...
    public User getUserByUsername(String username) {
        UserEntity user = userRepository.findByUsername(username).orElseThrow(
                () -> new NotFoundException("User with " + username + " not found", UserService.class.getSimpleName()));
     ...
     *
     * @param credentials
     *            New user data containing name, username and password
    ...
    public User createUser(UserRequest userRequest) {
        ...
        UserEntity newUser = UserEntity.builder().username(userRequest.getUsername())
                ...
                .role(userRequest.getRole() != null ? userRequest.getRole() : Role.ROLE_USER).build();
